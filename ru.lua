...
MainTab:Section({ 
    Title = "Приветствуем в EgasHub!",
    TextSize = 30,
	TextXAlignment = "Center"
})

MainTab:Section({ Title = "Скорость" })
...
LightTab:Button({
    Title = "ГДЕ ИРИСКИ?...",
    Desc = "изменяет все картины в игре на мемы с ириской",
    Callback = function()
...
GuiTab:Section({ Title = "Аддоны от Егаса" })

GuiTab:Button({
    Title = "[Б] Полет от меня",
    Desc = "Используй ПОСЛЕ первого этажа.",
    Callback = function()
loadstring(game:HttpGet("https://pastebin.com/raw/PDdkDsgy"))()
    end,
})

GuiTab:Section({ Title = "Другие хабы" })
...
FTab:Section({ 
    Title = "А: А где ключ система?",
    TextSize = 25,
})

FTab:Section({ 
    Title = "B: сьела ириска.", 
    TextSize = 30,
})

FTab:Section({ 
    Title = "A: Где я могу стать\nпереводчиком скрипта?", 
    TextSize = 25,
})

FTab:Section({ 
    Title = "B: Оставайся в крусе\n новостей о переводе в \ndiscord.gg/kotolog!", 
    TextSize = 30,
})
